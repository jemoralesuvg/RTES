...
#define COURSE_NO "2"
// Assignment number
#define ASSIGNMENT_NO "6"
// Macro which sends message to syslog with format required by course
#define SP_SYSLOG(...) syslog(LOG_CRIT,"[COURSE:" COURSE_NO "][ASSIGNMENT:" ASSIGNMENT_NO "]: " __VA_ARGS__)

void syslog_uname(){
     
   //clear syslog to have a syslog with only the logs of this program
   system("sudo sh -c '> /var/log/syslog'");
...
void main(void)
{
    ...
    clock_getres(MY_CLOCK_TYPE, &rt_res);
   ...
   printf("System has %d processors configured and %d available.\n", get_nprocs_conf(), get_nprocs());
    ...
    rc=pthread_create(&threads[4], &rt_sched_attr[4], Service_4, (void *)&(threadParams[3]));
    if(rc < 0)
...
void *Sequencer(void *threadp)
{
    ...
    do
    {
        ...
        {
#ifdef ABS_DELAY
            rc=clock_nanosleep(MY_CLOCK_TYPE, TIMER_ABSTIME, &delay_time, (struct timespec *)0);
#else
            rc=clock_nanosleep(MY_CLOCK_TYPE, 0, &delay_time, &remaining_time);
#endif
        ...
        current_time=getTimeMsec();
        //syslog(LOG_CRIT, "RTSEQ: cycle %08llu @ sec=%lf, dt=%lf\n", seqCnt, current_time, (current_time-last_time));
        ...
        // Servcie_1 = RT_MAX-1	
        if((seqCnt % T1) == 0) sem_post(&semS1);

        // Service_2 = RT_MAX-2	
        if((seqCnt % T2) == 0) sem_post(&semS2);

        // Service_3 = RT_MAX-3	
        if((seqCnt % T3) == 0) sem_post(&semS3);
        
        // Service_4 = RT_MAX-4	
        if((seqCnt % T4) == 0) sem_post(&semS4);
...
void waste_wcet_10ms (){
    long x = 0;
    for (int j = 0; j < 100; j++){  //WCET 10ms
...
void *Service_1(void *threadp)
{
    ...
    while(!abortS1)
    {
        ...
        current_time=getTimeMsec();
        SP_SYSLOG("Thread 1 start %d @ %6.9lf on core %d", S1Cnt, current_time, sched_getcpu() );
        ...
        current_time=getTimeMsec();
        //SP_SYSLOG("Thread 1 done %d @ %6.9lf on core %d", S1Cnt, current_time, sched_getcpu() );
    }
...
void *Service_2(void *threadp)
{
    ...
    while(!abortS2)
    {
        ...
        current_time=getTimeMsec();
        SP_SYSLOG("Thread 2 start %d @ %6.9lf on core %d", S2Cnt, current_time, sched_getcpu() );
        ...
        current_time=getTimeMsec();
        //SP_SYSLOG("Thread 2 done %d @ %6.9lf on core %d", S2Cnt, current_time, sched_getcpu() );
    }
...
void *Service_3(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        current_time=getTimeMsec();
        SP_SYSLOG("Thread 3 start %d @ %6.9lf on core %d", S3Cnt, current_time, sched_getcpu() );
        ...
        current_time=getTimeMsec();
        //SP_SYSLOG("Thread 3 done %d @ %6.9lf on core %d", S3Cnt, current_time, sched_getcpu() );
...
void *Service_4(void *threadp)
{
    ...
    while(!abortS4)
    {
        ...
        current_time=getTimeMsec();
        SP_SYSLOG("Thread 4 start %d @ %6.9lf on core %d", S4Cnt, current_time, sched_getcpu() );
        ...
        current_time=getTimeMsec();
        //SP_SYSLOG("Thread 4 done %d @ %6.9lf on core %d", S3Cnt, current_time, sched_getcpu() );
