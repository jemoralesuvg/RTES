...
// Function to set the scheduler policy
void set_scheduler(void)
{
    int max_prio, scope, rc, cpuidx;
    cpu_set_t cpuset;

    // print initial scheduler policy
    printf("INITIAL "); print_scheduler(); 
    
    // Initialize the thread attribute with default values
    pthread_attr_init(&fifo_sched_attr);
    // Set explicit scheduling attribute inheritance
    pthread_attr_setinheritsched(&fifo_sched_attr, PTHREAD_EXPLICIT_SCHED);
    // Set the scheduling policy of the attribute to SCHED_POLICY
    pthread_attr_setschedpolicy(&fifo_sched_attr, SCHED_POLICY);
    
    CPU_ZERO(&cpuset);  // Clear the CPU set
    cpuidx=(3);         // Set the CPU index to be used
    CPU_SET(cpuidx, &cpuset); // Add the CPU index to the CPU set
    // Set the CPU affinity of the thread attribute to the CPU set
    ...
    // Set the scheduling priority of the FIFO scheduler parameters
    fifo_param.sched_priority=max_prio;    

    // Set the scheduling policy and parameters for the calling process
    if((rc=sched_setscheduler(getpid(), SCHED_POLICY, &fifo_param)) < 0)
        perror("sched_setscheduler"); 
    
    // Set the scheduling parameters of the thread attribute
...
// Thread function for performing the counting task
void *counterThread(void *threadp)
{
	...
	SP_SYSLOG("Thread idx=%d, sum[1...%d]=%d Running on core:%d", 
           threadParams->threadIdx,
           threadParams->threadIdx+1, sum, sched_getcpu());
           /*
...
// Starter thread function for creating counter threads
void *starterThread(void *threadp)
{
   int i, rc;

   printf("starter thread running on CPU=%d\n", sched_getcpu());
    // create counter threads according to NUM_THREADS
...
int main (int argc, char *argv[])
{
   ...
   else
   {
    ...
    // create starter thread that will start multiple counter threads
